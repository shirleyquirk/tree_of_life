...
public final class Noise {
    ...
    static private final int P = 8;
    static private final int B = 1 << P;
    static private final int M = B-1;

    static private final int NP = 8;
    static private final int N = 1 << NP;
    static private final int NM = N-1;
    ...
    static private double s_curve(double t) {
       t = t + t;
       return (t < 1 ? Math.pow(t, 2.25) : 2-Math.pow(2-t, 2.25)) / 2;
    }
    ...
    static double[] G(int i) { return points[i % 32]; }

    static double[][] points = new double[32][3];
    static boolean initPoints = initPoints();
    static boolean initPoints() {
       points[3][0] = points[3][1] = points[3][2] = Math.sqrt(1./3);
       double r = Math.sqrt(1./2);
       double s = Math.sqrt(2 + r + r);

       for (int i = 0 ; i < 3 ; i++)
       for (int j = 0 ; j < 3 ; j++)
          points [i][j] = ( i == j ? 1 + r + r : r ) / s;
       for (int i = 0 ; i <= 1 ; i++)
       for (int j = 0 ; j <= 1 ; j++)
       for (int k = 0 ; k <= 1 ; k++) {
          int n = i+j*2+k*4;
       ...
       for(i=0; i<B ; i++) {
          ...
          do {
             ...
             Hi = Math.max(U, Math.max(V, W));
              ...
              Math.min(Math.abs(U-V), Math.min(Math.abs(U-W),
	                                       Math.abs(V-W))) < .2);
       }
