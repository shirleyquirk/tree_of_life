...
#-28.0_t2=84.0_thet=((0.699_0.869)(1.05_1.064)_len=(0.79_0.69)
coeffs=[[-28,84,((0.699,0.869),(1.05,1.064)),(0.79,0.69)]]
...
def generate_with_coefs(c,double=False):
    ...
    for i in range(10,14):
        ps.append(Process(target=generate_a_tree,args=('Mesomycetozoa_'+'_'.join([str(i),str(NumBranches),str(t1),str(t2),str((tuple(thm.l),tuple(thm.r))),str(tuple(lenm))]),i,NumBranches,'poobumpants',t1,t2,thm,lenm)))
        ps[-1].start()
    for p in ps:
        p.join()
    ...
    for f in filenames:
        ...
        DPI='30'
        tmpfile=f
        destfile='Ichthy/'+fileprefix+'/'+fileprefix+'.svg'
        destdir='Ichthy/'+fileprefix+'/'
        names=['ichthytree_'+str(idx) for idx in range(NumBranches)]
        for n in names:
            #x0=coefs[n]['x']
            #y0=coefs[n]['y']
