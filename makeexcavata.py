...
#t1=-30.0_t2=75.0_thet=((0.695_0.911)(1.05_1.038)_len=(0.81_0.63)
coeffs=[[-30,75,((0.695,0.911),(1.05,1.038)),(0.81,0.63)]]
...
def generate_with_coefs(c,double=False):
    ...
    for i in range(11,14):
        ps.append(Process(target=generate_a_tree,args=('Excavata_'+'_'.join([str(i),str(NumBranches),str(t1),str(t2),str((tuple(thm.l),tuple(thm.r))),str(tuple(lenm))]),i,NumBranches,'poobumpants',t1,t2,thm,lenm)))
        ps[-1].start()
    for p in ps:
        p.join()
    ...
    for f in filenames:
        ...
        DPI='30'
        tmpfile=f
        destfile='Excavata/'+fileprefix+'/'+fileprefix+'.svg'
        destdir='Excavata/'+fileprefix+'/'
        names=['excavatree_'+str(idx) for idx in range(NumBranches)]
        for n in names:
            #x0=coefs[n]['x']
            #y0=coefs[n]['y']
