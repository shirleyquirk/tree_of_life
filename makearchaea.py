...
#-26.0_t2=78.0_thet=((0.744_0.845)(1.05_0.976)_len=(0.81_0.68)
#-32.0_t2=78.0_thet=((0.738_0.897)(1.05_0.951)_len=(0.8_0.63)
coeffs=[[-26,78,((0.744,0.845),(1.05,0.976)),(0.81,0.68)],\
    [-32,78,((0.738,0.897),(1.05,0.951)),(0.8,0.63)]]
...
def generate_with_coefs(c,double=False):
    ...
    for i in range(11,14):
        ps.append(Process(target=generate_a_tree,args=('Archaea_'+'_'.join([str(i),str(NumBranches),str(t1),str(t2),str((tuple(thm.l),tuple(thm.r))),str(tuple(lenm))]),i,NumBranches,'poobumpants',t1,t2,thm,lenm)))
        ps[-1].start()
    for p in ps:
        p.join()
    ...
    for f in filenames:
        ...
        DPI='30'
        tmpfile=f
        destfile='Archaea/'+fileprefix+'/'+fileprefix+'.svg'
        destdir='Archaea/'+fileprefix+'/'
        names=['archeatree_'+str(idx) for idx in range(NumBranches)]
        for n in names:
            #x0=coefs[n]['x']
            #y0=coefs[n]['y']
