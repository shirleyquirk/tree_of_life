...
#-28.9_75.8_((0.683, 0.847), (1.33, 0.849))_(0.81, 0.78)
coeffs=[[-28.9,75.8,((0.683,0.847),(1.33,0.849)),(0.81,0.78)]]
...
def generate_with_coefs(c,double=False):
    ...
    for i in range(11,15):
        ps.append(Process(target=generate_a_tree,args=('SAR_'+'_'.join([str(i),str(NumBranches),str(t1),str(t2),str((tuple(thm.l),tuple(thm.r))),str(tuple(lenm))]),i,NumBranches,'poobumpants',t1,t2,thm,lenm)))
        ps[-1].start()
    for p in ps:
        p.join()
    ...
    for f in filenames:
        ...
        DPI='30'
        tmpfile=f
        destfile='SAR/'+fileprefix+'/'+fileprefix+'.svg'
        destdir='SAR/'+fileprefix+'/'
        names=['sartree_'+str(idx) for idx in range(NumBranches)]
        for n in names:
            #x0=coefs[n]['x']
            #y0=coefs[n]['y']
